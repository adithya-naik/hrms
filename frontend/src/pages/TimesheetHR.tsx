...
const TimesheetHR: React.FC = () => {
  ...
  // Fetch employees, projects, modules
  useEffect(() => {
  ...
  const fetchTimesheets = async () => {
    ...
    try {
      ...
      const res = await axios.get("http://localhost:5000/api/hr/timesheets", {
              ...
              {loading ? (
                ...
                timesheets.map(ts => (
                  <TableRow key={ts.id}>
                    ...
                    <TableCell>
                      <Button size="sm" onClick={() => setSelectedTimesheet(ts)}>Overview</Button>
                    </TableCell>
                  </TableRow>
                ))
              )}
